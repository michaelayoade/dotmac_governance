# 0013. Strict-xfail plus an absence ratchet is valid only for a staged specification
...
Starter ADR 0018 already established the fleet exemption rule this depends on:
...
2. **An unmarked proof that the dependency is still absent.** Unmarked is
   ...
   the markers cannot be forgotten. This is ADR 0018's two-directional ratchet
   applied to staging.
