...
| 19 | Where an authority-cutover receipt is stored, and who owns that store | ADR 0018 rule 1 requires an immutable receipt that outlives both parties and stays addressable after the counterparty is gone. It does not name the store. A receipt held only in the system losing authority is not durable, and the fleet has no cross-repository evidence store today. The store, its retention, and its owner must be decided before the rule can be claimed as implemented. | Michael |
