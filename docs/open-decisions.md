...
| 21 | Receipt-registry directory, envelope schema and append-only enforcement | ADR 0018 § 3 decides the owner, the envelope contents and the discipline; it deliberately does not create the store. The registry directory, its strict parser, its append-only validator (comparing an existing receipt's bytes against the merge base, never the diff's shape), and whether the envelope is represented in `standards-profile.schema.json` are a separate reviewed change. Until it lands there is no registry and no receipt may be written to one. | Michael |
