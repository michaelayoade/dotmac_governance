# 0024. Fleet integration architecture standard: ingress eligibility, event identity, and retention lifetimes
...
request #17 under the number 0012. It never merged. ADR 0012 was taken in the
meantime by the Dotmac ISP replacement programme, which is `Accepted` and owns
`programmes/dotmac-isp-replacement.json`; under `docs/adr/README.md` the record
that merges first keeps the number, so this one renumbers. The decision text is
carried over unchanged, because a census against every `Accepted` record found
nothing on `main` that supersedes any part of it: the words *handshake*,
*provider event identity* and *tombstone* appear in no merged governance record.
Pull request #17 is closed in favour of this one.

**Its observations are as-of 2026-08-16.** ADR 0013 § 4 was accepted after this
text was written. Every product citation below — file, line, revision and merge
...
branch before it is used to size any work.
...
Those are open decisions, recorded in `docs/open-decisions.md` as decision 27
