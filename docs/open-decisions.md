...
| 25 | Owner and interval for the recovery-bundle rehearsal | ADR 0022 (`Proposed`) assigns the bundle format, closure computation and rehearsal harness to `dotmac-deployment-foundation` and the rehearsal trigger to deployment control. Neither repository has accepted that assignment, and no rehearsal interval is set. The credential half is **resolved**: Michael approved the post-restore binding model on 2026-08-30 and it is recorded as ADR 0022 § 5. Every existing fleet backup remains a data copy until this is resolved, and every recovery previously recorded `PROVED` under the schema/row-count/heads definition is reclassified **unproven** rather than failed. `PostgresRecoveryBundleV1` (Starter #518, `d6b9aae5`) is the reference implementation; it is not fleet coverage. | Michael |
