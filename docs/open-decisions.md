...
| 25 | Owner and interval for the recovery-bundle rehearsal | ADR 0022 (`Proposed`) assigns the bundle format, closure computation and rehearsal harness to `dotmac-deployment-foundation` and the rehearsal trigger to deployment control. Neither repository has accepted that assignment, no rehearsal interval is set, and § 2 deliberately excludes passwords from the bundle without deciding how credentials are re-supplied at recovery time. Every existing fleet backup is a data copy until this is resolved, and every recovery previously recorded `PROVED` under the schema/row-count/heads definition is reclassified **unproven** rather than failed. `PostgresRecoveryBundleV1` (Starter #518, `d6b9aae5`) is the reference implementation; it is not fleet coverage. | Michael |
