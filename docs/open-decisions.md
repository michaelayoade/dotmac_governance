...
| 13 | What ADR 0012 (fleet integration architecture standard) is enforced by, and by whom | Four sub-items, none inferred by the drafting agent. (a) Whether the three standards become a `standards_control` rule family under ADR 0006 — and at which schema version — or stay a review-only obligation; a `Proposed` record cannot be `required`, so this cannot be built before approval. (b) The replay-evidence retention period: Michael has stated 180 days from `received_at`, but the controlled record still carries `replay_evidence_period = "UNSET"`, no second-phase retirement exists in code, and DPO acceptance of the actual notice/ROPA revision is `PENDING`. Setting the number without the control would claim an enforcement that does not exist. (c) The named repair owner and migration path for Sub's digest-keyed provider identity (`app/api/inbox_webhooks.py:277`), including identities already written in the digest form. (d) `dotmac_starter_mt`, which carries every reference implementation cited by ADR 0012, is out of initial governed scope under decision 9; the standard cannot bind a repository whose enrollment is undecided. | Michael |
